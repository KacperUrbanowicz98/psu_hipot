...
class TestScreen:
    ...
    def create_device_info(self):
        info_frame = tk.Frame(
            self.main_frame, bg=self.config.COLOR_WHITE,
    ...
    def create_live_display(self):
        ...
        self.voltage_label = tk.Label(vf, text="0 V", bg=self.config.COLOR_WHITE,
                                      fg=self.config.COLOR_PRIMARY, font=("Arial", 32, "bold"))
        ...
        self.current_label = tk.Label(cf, text="0.00 mA", bg=self.config.COLOR_WHITE,
                                      fg=self.config.COLOR_ACCENT, font=("Arial", 32, "bold"))
        ...
        try:
            ...
            while self.test_running:
                ...
                if measurements:
                    ...
                    self.current_current = measurements['measure_current'] * 1000  # A → mA
    ...
    def show_result_and_next_serial(self, result, data):
        ...
        tk.Button(
            ...
            relief=tk.FLAT, cursor="hand2", command=next_test
        ).pack(side=tk.LEFT, padx=5)

        tk.Button(
            btn_frame, text="Powrót do menu",
            bg=self.config.COLOR_PRIMARY, fg=self.config.COLOR_WHITE,
            font=("Arial", 12, "bold"), width=14, height=2,
            relief=tk.FLAT, cursor="hand2", command=back_to_menu
        ).pack(side=tk.LEFT, padx=5)